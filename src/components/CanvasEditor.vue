    ...
    <div class="design-wrapper d-flex">
      <div class="editor-wrapper" id="editor-wrap">
        ...
        <div class="font-wrapper">
          ...
          <i class="fas fa-save" @click="saveGraph"></i>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    saveGraph() {
      alert("thanks");
      this.graphTwo.set("graphCustomProperty", true);
      this.graphTwo.set("graphExportTime", Date.now());
      var jsonObject = this.graphTwo.toJSON();

      this.graph.set("graphCustomProperty", true);
      this.graph.set("graphExportTime", Date.now());
      var jsonObject1 = this.graph.toJSON();
  ...
  created() {
    ...
    const standard = this.$joint.shapes;
    ...
    // console.log(this.$lodi.isEmpty() ? "Lodash is available here!" : "Uh oh..");
    console.log("console logging dis in created()", this);

    // this.svgZoom = svgPanZoom("#main-wrap svg", {
    //   center: false,
    //   zoomEnabled: true,
    //   panEnabled: true,
  ...
  mounted() {
    ...
    // ---------------------------------------------------
    console.log("console logging dis in mounted()", this);
    var philip = this;
    console.log("console logging philip in mounted()", philip);
